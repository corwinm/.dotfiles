...
The large Apple logo at the far left has no pill background, turns white in dark mode, and opens a short system menu for About This Mac, System Settings, Activity Monitor, and locking. Clicking the current-application capsule opens shortcuts for a new window, application settings, hiding, and quitting; these send the standard macOS Command-key shortcuts to the frontmost application and require Accessibility access for SketchyBar. The coding-agent capsule opens an indexed details popup with agent type, project, state, focused-pane highlighting, direct pane switching, and a link to the full chooser. Both popups dismiss when the pointer leaves them.
